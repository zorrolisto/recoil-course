...
const selectedElementProperties = selector<Element | null>({
    ...
    get: ({get}) => {
        const selectElementId = get(selectedElementState)
        if (selectElementId === null) return null

        return get(elementState(selectElementId))
    },
    set: ({get, set}, newElement) => {
        const selectElementId = get(selectedElementState)
        if (selectElementId === null || !newElement) return null

        if ('style' in newElement) set(elementState(selectElementId), newElement)
    },
})
    ...
    return (
        <Card>
            ...
            <Section heading="Size">
                <Property label="Width" value={element.style.size.width} onChange={
